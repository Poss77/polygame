-- ==============================================================================
-- POLYGAME SECURITY HARDENING MIGRATION
-- File: seal_referrals_and_drift_security_shield.sql
-- Date: 2026-09-16
-- 
-- Fixes & Protections:
-- 1. Anti-Cheat Master Trigger: Seals direct client writes to referrals_count,
--    referrals_l1..l4, and referrals_list (strictly SECURITY INVOKER).
-- 2. claim_faucet & claim_vip_faucet: Queries true registered downline accounts
--    directly from users table, making faucet referral bonus 100% server-authoritative.
-- 3. end_arcade_session: Enforces physical duration rate-clamps on orbs (max 3/sec),
--    bonus tokens (max 1/15sec), and arcade score velocity (max 350 pts/sec).
-- 4. Reconciles existing referral tree counts across all users.
-- ==============================================================================

-- Drop existing function overloads to prevent parameter name conflict (42P13)
...
-- Reconcile historical referral tree counters across database
SELECT public.reconcile_referral_trees();
