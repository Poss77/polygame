-- ====================================================================
-- INVESTIGATION QUERY: HOW DID GINZA SPEND / LOSE PGT?
-- Run this in Supabase SQL Editor to inspect Ginza's full history.
-- ====================================================================

-- 1. IDENTIFY GINZA'S PROFILE, CURRENT BALANCE & TOTAL STATS
SELECT 
  player_id,
  username,
  ...
  relics
FROM users
WHERE LOWER(username) LIKE '%ginza%' 
   OR LOWER(player_id) LIKE '%ginza%'
   OR LOWER(COALESCE(linked_wallet_address, '')) LIKE '%ginza%';
...
-- 3. ACTIVE & HISTORICAL STAKING POSITIONS (PGT Moved to Vault)
SELECT 
  id,
  player_id,
  amount_pgt,
