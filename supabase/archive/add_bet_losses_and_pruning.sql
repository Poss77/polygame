-- ==============================================================================
-- MIGRATION: ADD BET LOSSES, OUTCOME TRACKING & RETENTION PRUNING
-- Version: v1.5.418
-- Description:
--   1. Adds 'outcome' ('win', 'loss', 'push') and 'profit_loss' columns to bet_wins.
--   2. Backfills historical rows with outcome = 'win'.
--   3. Updates claim_daily_quest to ensure Daily Quest 2 only counts actual wins.
--   4. Creates secure prune_old_bet_wins(p_days, p_admin_passkey) RPC.
-- ==============================================================================

-- 1. ADD NEW COLUMNS TO bet_wins
ALTER TABLE public.bet_wins ADD COLUMN IF NOT EXISTS outcome TEXT DEFAULT 'win';
ALTER TABLE public.bet_wins ADD COLUMN IF NOT EXISTS profit_loss NUMERIC DEFAULT 0;

-- 2. BACKFILL HISTORICAL ROWS
UPDATE public.bet_wins
SET outcome = 'win',
    profit_loss = COALESCE(payout, 0) - COALESCE(bet_amount, 0)
WHERE outcome IS NULL;

-- 3. CREATE PERFORMANCE INDEX FOR PRUNING AND QUERIES
...
-- 4. UPDATE claim_daily_quest TO SAFEGUARD QUEST 2 AGAINST LOSSES
...
-- 5. RPC: prune_old_bet_wins
-- Description: Prunes historical wager & bet logs older than p_days (default: 30 days)
-- Requires Master Admin Passkey verification for security.
