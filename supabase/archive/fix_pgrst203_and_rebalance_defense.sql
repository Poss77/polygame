-- ==============================================================================
-- POLYGON GAMING: RESOLVE PGRST203 FUNCTION OVERLOADING COLLISION
-- & DEPLOY REBALANCED CYBER DEFENSE 2X PAYOUT RPC
-- ==============================================================================
-- Root Cause: PostgreSQL had two conflicting overloaded versions of end_arcade_session
-- (one starting with p_player_id, and one starting with p_session_id).
-- This script purges all duplicate signatures and installs the single canonical function.
-- ==============================================================================

-- 1. Dynamically drop every existing overloaded signature of end_arcade_session
DO $DROP$
DECLARE
  r RECORD;
BEGIN
...
-- 3. Install the SINGLE CANONICAL end_arcade_session function
...
BEGIN
  ...
  ELSIF v_game_clean LIKE '%defense%' THEN
    ...
    -- Rebalanced: Reduced by 2x ((score / 4000.0) + (creeps * 0.025))
