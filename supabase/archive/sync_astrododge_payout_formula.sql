-- ==============================================================================
-- FIX: SYNCHRONIZE ASTRODODGE ARCADE PAYOUT FORMULA
-- ==============================================================================
-- Synchronizes AstroDodge formula from (score / 1000.0) to (score / 2500.0) + (shards * 0.05)
-- to match the client HUD and game rules exactly.
-- ==============================================================================

DROP FUNCTION IF EXISTS public.end_arcade_session(UUID, INTEGER, INTEGER, INTEGER, NUMERIC) CASCADE;
DROP FUNCTION IF EXISTS public.end_arcade_session(TEXT, INTEGER, INTEGER, INTEGER, NUMERIC) CASCADE;
...
DECLARE
  ...
  v_clamped_nft_mult NUMERIC := GREATEST(1.0, LEAST(10.0, COALESCE(p_nft_multiplier, 1.0)));
...
BEGIN
  -- Safe session UUID parsing
  BEGIN
    v_session_uuid := p_session_id::UUID;
  EXCEPTION WHEN OTHERS THEN
    RETURN jsonb_build_object('success', false, 'error', 'Invalid session UUID format');
  END;

  -- Lock active arcade session
