-- ============================================================================
-- POLYGON GAMING: MIGRATE ARCADE PAYOUT CEILING TO BASE GAMEPLAY (v_raw_pgt)
-- ============================================================================
-- Version: v1.5.347
-- Purpose:
--   1. Shifts the arcade payout ceiling from the post-multiplier final reward
--      down to the BASE gameplay earn (v_raw_pgt <= 75.00 PGT).
--   2. Why this is superior:
--      - 3.3x Tighter Against Exploits: An unmultiplied player/bot can earn at
--        most 75.00 PGT per run (reduced from 250.00 PGT).
--      - 100% Fair for Bonus Stacking: Players who invested in VIP (2.0x),
--        Ambassador (2.0x), Apex Relics (1.5x), and Utility NFTs (up to 2.0x)
--        NEVER hit an artificial brick wall. Their multipliers apply freely
--        to their base earn up to legitimate high-score rewards (e.g. 75 * 12 = 900 PGT).
--   3. Preserves all anti-cheat velocity clamps (v_max_velocity_rate) and
--      ultra-fast session protections (< 3s clamped to max 1.00 PGT).
--   4. Adds a generous catastrophe circuit-breaker on v_final_pgt (1,000.00 PGT).
-- ============================================================================
...
-- 2. CREATE RE-CALIBRATED end_arcade_session PROCEDURE
...
BEGIN
  ...
  -- 7. Calculate Game-Specific Base PGT Formula
  ...
  ELSE
    ...
    -- ========================================================================
    -- ⭐ BASE GAME EARN CEILING (75.00 PGT):
    -- ========================================================================
    -- Capping base rewards at 75.00 PGT is ~3.3x tighter against unmultiplied
    -- bots & exploits (max 75 PGT vs previous 250 PGT), while ensuring that
    -- players who invested in VIP (2x), Ambassador (2x), Relics (1.5x), and
    -- NFTs (up to 2x) are NEVER penalized for their bonuses!
    -- ========================================================================
    v_raw_pgt := LEAST(v_raw_pgt, 75.00);

    -- Apply player's verified total multiplier to the capped base + add token bonus
...
-- 3. GRANT PERMISSIONS & RELOAD SCHEMA CACHE
