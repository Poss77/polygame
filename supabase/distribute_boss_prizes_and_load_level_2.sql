-- ==============================================================================
-- POLYGAME: DISTRIBUTE LEVIATHAN PRIZES & LOAD LEVEL 2 (7,500,000 HP • 12,000 PGT)
-- ==============================================================================
-- 1. Distributes the 10,000 PGT Level 1 prize pool to all 10 Boss hunters based
--    on their verified career boss damage.
-- 2. Sets the active Quantum Leviathan to LEVEL 2 (7,500,000 Max HP • 12,000 PGT Pool).
-- 3. Resets active weekly damage to 0 so all commanders start fresh against Level 2.
-- 4. Updates the distribute_weekly_boss_prizes() RPC for future Sunday resets.
-- ==============================================================================

DO $$
DECLARE
  v_pool NUMERIC := 10000.0;
  v_total_dmg NUMERIC := 0.0;
  v_hunter RECORD;
  v_payout NUMERIC;
  v_distributed_total NUMERIC := 0.0;
  v_count INT := 0;
  v_settings JSONB;
BEGIN
  -- 1. Calculate total boss damage dealt across all players
  SELECT COALESCE(SUM(alltime_boss_damage), 0) INTO v_total_dmg
  FROM public.users
  WHERE alltime_boss_damage > 0;

  IF v_total_dmg > 0 THEN
    -- 2. Distribute proportional 10,000 PGT Level 1 pool to each commander
    FOR v_hunter IN
      SELECT player_id, username, alltime_boss_damage
      FROM public.users
      WHERE alltime_boss_damage > 0
      ORDER BY alltime_boss_damage DESC
    LOOP
      v_payout := ROUND((v_hunter.alltime_boss_damage / v_total_dmg) * v_pool, 2);
      ...
      IF v_payout > 0 THEN
        ...
        SET 
          ...
          boss_weekly_damage = 0,
          updated_at = NOW()
        WHERE player_id = v_hunter.player_id;

        v_distributed_total := v_distributed_total + v_payout;
        v_count := v_count + 1;
        
        RAISE NOTICE 'Awarded % PGT to % (%) for % DMG', v_payout, v_hunter.username, v_hunter.player_id, v_hunter.alltime_boss_damage;
      END IF;
    END LOOP;
  END IF;

  -- 3. Fetch and update game_payout_settings in global_settings for Level 2 (12,000 PGT Pool)
  ...
  -- 4. Load LEVEL 2 into global_settings: 7,500,000 HP (+50%) and 12,000 PGT Pool (+20%)
  ...
  RAISE NOTICE 'SUCCESS: Distributed % PGT to % commanders. Quantum Leviathan ascended to LEVEL 2 (7,500,000 HP • 12,000 PGT Pool).', v_distributed_total, v_count;
END $$;

-- 5. Recreate distribute_weekly_boss_prizes RPC for automated weekly resets
