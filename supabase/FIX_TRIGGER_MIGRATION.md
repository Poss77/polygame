# PolyGame Security Shield & Trigger Hotfix Migration

**File**: `supabase/FIX_TRIGGER_MIGRATION.md`  
**Related SQL**: [`supabase/fix_trigger_restore_full_shield.sql`](./fix_trigger_restore_full_shield.sql)  
**Date**: 2026-09-17  
...
When applying the update to `prevent_direct_balance_mutation`, the following error occurred in `state.js:477`:
```text
PATCH https://jgtfnsufemvqkyytscgl.supabase.co/rest/v1/users ... 400 (Bad Request)
{ code: '42703', message: 'record "new" has no field "balance_1flr"' }
```

### Root Cause
1. **Dropped Column**: `balance_1flr` was a legacy token column that was dropped from `public.users` in an earlier cleanup migration. Attempting to access or assign `NEW.balance_1flr` fails because the column no longer exists.
2. **Function Overwrite**: In PostgreSQL, `CREATE OR REPLACE FUNCTION` replaces the entire function body. A partial snippet unintentionally displaced the other 12 security shields.

---

## SQL Script to Execute in Supabase SQL Editor

Copy and execute the entire block below in your **Supabase SQL Editor**:
