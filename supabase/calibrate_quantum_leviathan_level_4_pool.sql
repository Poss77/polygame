-- ==============================================================================
-- POLYGAME: CALIBRATE LEVEL 4 QUANTUM LEVIATHAN POOL & HARDEN WEEKLY RESET
-- Target: Level 4 Quantum Leviathan (16,875,000 HP • 17,280 PGT Weekly Pool)
-- ==============================================================================

-- 1. Calibrate global_settings with active Level 4 pool (17,280 PGT)
DO 
DECLARE
  v_settings JSONB;
BEGIN
  ...
  IF v_settings->'boss' IS NULL THEN
    v_settings := jsonb_set(v_settings, '{boss}', '{name: 👾 Cosmic World Boss (Quantum Leviathan), vip_only: false, test_mode: false, harvest_enabled: true, leaderboard_enabled: true}'::jsonb);
  END IF;

  -- Set Level 4 Pool: 10,000 * 1.20^3 = 17,280 PGT
  v_settings := jsonb_set(v_settings, '{boss,weekly_pool_pgt}', '17280'::jsonb);
  ...
  RAISE NOTICE 'SUCCESS: Quantum Leviathan pool calibrated to 17,280 PGT for Level 4.';
END ;

-- 2. Upgrade distribute_weekly_boss_prizes() to ensure level-scaled pool is never downgraded
...
BEGIN
  ...
  ELSE
    ...
    RETURN jsonb_build_object(
      ...
      'slain', false,
      'distributed', false,
      'message', 'Quantum Leviathan survived! Prize pool withheld and Boss reset to Level 1.',
      'survived_level', v_boss_level,
      'boss_level', v_boss_level,
      'remaining_hp', v_boss_current_hp,
      'next_level', 1,
      'new_level', 1,
      'winner_count', 0,
      'payout_count', 0,
