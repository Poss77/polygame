...
export async function syncProfileWithDb(address, pgtBalance, flrBalance, maticBalance, chainNfts, silent = false) {
    ...
    if (supabase) {
      ...
      if (data && !error) {
        ...
        // Maximize daily quest progress so quest counters NEVER revert & sync to DB if local progress is ahead
        const today = new Date().toISOString().split('T')[0];
        let localQ = activeAppState.state.dailyQuests;
        if (!localQ || typeof localQ !== 'object' || !localQ.date) {
          try {
            const ls = localStorage.getItem('polygame_daily_quests');
            if (ls) localQ = JSON.parse(ls);
          } catch(e) {}
        }
        localQ = localQ || {};

        const dbQ = (data.daily_quests && typeof data.daily_quests === 'object' && Object.keys(data.daily_quests).length > 0)
          ? data.daily_quests
          : null;

        let needsDbSync = false;

        if (dbQ && dbQ.date === today && localQ.date === today) {
          ...
          activeAppState.state.dailyQuests = {
            ...
            mining: mergedMining,
            wins: mergedWins,
            games_claimed: mergedGamesClaimed,
            mining_claimed: mergedMiningClaimed,
            wins_claimed: mergedWinsClaimed,
            master_claimed: mergedMasterClaimed,
        ...
        try {
          ...
          const genCode = Math.random().toString(16).substring(2, 10);
      ...
      setTimeout(async () => {
        try {
          ...
          if (Array.isArray(chainNftsList)) {
            ...
            if (currentStr !== chainStr) {
              ...
              if (supabase && onchainTargetAddress) {
                ...
                // Cryptographically sync on-chain assets via secure Edge Function
                supabase.functions.invoke('sync-assets', {
                  body: {
                    playerId: targetPId,
                    walletAddress: onchainTargetAddress
                  }
                }).then(fnRes => {
                  if (fnRes && fnRes.data && fnRes.data.success) {
                    ...
                    if (window.POLY_DEBUG) console.log("[syncProfileWithDb] On-chain assets authoritatively verified & synced via sync-assets Edge Function.");
                  }
                }).catch(fnErr => {
                  if (window.POLY_DEBUG) console.warn("[syncProfileWithDb] sync-assets notice:", fnErr);
                });
              }
              if (typeof window.renderNftInventory === 'function') window.renderNftInventory();
              if (typeof window.syncProfileView === 'function') window.syncProfileView();
          ...
          if (chainRelicsObj && typeof chainRelicsObj === 'object') {
            ...
            // 2. Overlay on-chain counts & token_ids without wiping unminted relics
  ...
  try {
    ...
    if (userRow) {
      ...
      if (!validGoogleRefCode || validGoogleRefCode.trim() === '' || validGoogleRefCode === 'EMPTY') {
        validGoogleRefCode = Math.random().toString(16).substring(2, 10);
      ...
      // Master Admin Privileges strictly require active cryptographic Web3 wallet
      const adminNav = document.getElementById('nav-item-admin');
      const adminCard = document.getElementById('profile-admin-card');
      ...
      const isMasterAdminActive = (injected && injected === expectedAdmin);
