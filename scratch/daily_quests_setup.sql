-- ============================================================
-- POLYGAME DAILY QUESTS & LOGIN STREAK SYSTEM
-- Atomic rewards processing & daily progress tracking
-- Run this in your Supabase SQL Editor
-- ============================================================

-- Add daily_quests column to users table
...
-- Database RPC to claim quest reward
...
BEGIN
  ...
  IF COALESCE(v_quests->>'date', '') <> v_today THEN
    v_quests := jsonb_build_object(
      'date', v_today,
      'faucet', COALESCE((v_quests->>'faucet')::BOOLEAN, false),
      'games', 0,
      'mining', 0,
      'faucet_claimed', false,
      'games_claimed', false,
  ...
  IF p_quest_type = 'faucet' THEN
    v_already_claimed := COALESCE((v_quests->>'faucet_claimed')::BOOLEAN, false);
    IF NOT COALESCE((v_quests->>'faucet')::BOOLEAN, false) THEN
      RETURN jsonb_build_object('success', false, 'message', 'Claim your Faucet first today!');
    END IF;
    IF v_already_claimed THEN
      RETURN jsonb_build_object('success', false, 'message', 'Faucet quest reward already claimed today');
    END IF;
    v_reward := 15;
    v_quests := jsonb_set(v_quests, '{faucet_claimed}', 'true'::jsonb);

  ELSIF p_quest_type = 'games' THEN
    ...
    IF v_progress < 3 THEN
      RETURN jsonb_build_object('success', false, 'message', 'Play at least 3 Arcade rounds first!');
    END IF;
    IF v_already_claimed THEN
  ...
  ELSIF p_quest_type = 'master' THEN
    IF NOT (COALESCE((v_quests->>'faucet_claimed')::BOOLEAN, false) 
            AND COALESCE((v_quests->>'games_claimed')::BOOLEAN, false) 
            AND COALESCE((v_quests->>'mining_claimed')::BOOLEAN, false)) THEN
