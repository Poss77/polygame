-- ============================================================
-- POLYGAME REFERRAL TREE CLEANUP & REPAIR (CORRECTED)
-- 1. Replaces raw EVM addresses with player_ids (0xpgt...)
-- 2. Sets Master Admin (0x10b...) referred_by_l1 = Poss (0xpgt8312e02d...)
-- 3. Clears Poss's own referrer (Poss is root founder, no referrer)
-- 4. Correctly propagates L1 -> L2 -> L3 -> L4 (Poss's direct referrals get L2=NULL)
-- 5. Recalculates all referral counts across the 4-tier network
-- Run this in your Supabase SQL Editor
-- ============================================================

-- 1. Ensure Poss (0xpgt8312e02d37185b5983e6922d1dae1cce / 0x92206284cae2b1be18c8bcc9042ee) has NO referrer
UPDATE users
...
-- 2. Ensure Master Admin (0xpgt85c8416473bd6a8c45ada81ac85aeabb / 0x10b9993990c9ef8a212c9557cb02ad94da9a654d) is referred by Poss
UPDATE users
...
-- 3. Replace any raw EVM wallet addresses in referred_by columns with resolved player_id
DO $$
DECLARE
  r RECORD;
  v_res1 TEXT;
...
-- 5. Recalculate 4-Tier referral counts for all users
UPDATE users u SET
...
BEGIN
  ...
  -- Search referral_code first, then player_id, then linked_wallet_address
  ...
  -- ALWAYS store v_ref_user.player_id in referred_by_l1
  UPDATE users
